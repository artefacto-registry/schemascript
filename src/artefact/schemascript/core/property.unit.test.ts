...
describe("makeBuilder", () => {
	test("should wrap a property and return a builder", () => {
		const prop = new Property("text");
		const builder = makeBuilder(prop);
		expect(typeof builder).toBe("function");
		expect(builder()).toBe(prop);
	});

	test("should support chained modifiers on the builder", () => {
		const prop = new Property("text");
		const builder = makeBuilder(prop);

		const chained = builder.optional().unique().array();
		const finalProp = chained();

		expect(finalProp.isOptional).toBe(true);
		expect(finalProp.isUnique).toBe(true);
		expect(finalProp.isArray).toBe(true);
	});

	test("should support identifier modifier on the builder", () => {
		const prop = new Property("integer");
		const builder = makeBuilder(prop);

		const idBuilder = builder.identifier({ autoIncrement: true });
		const finalProp = idBuilder();

		expect(finalProp.isIdentifier).toBe(true);
		expect(finalProp.isAutoIncrement).toBe(true);
	});

	test("should support default modifier on the builder", () => {
		const prop = new Property("text");
		const builder = makeBuilder(prop);

		const defBuilder = builder.default("hello");
		const finalProp = defBuilder();

		expect(finalProp.hasDefault).toBe(true);
		expect(finalProp.defaultValue).toBe("hello");
	});
	...
	test("should support references modifier on the builder", () => {
		...
		expect(finalProp.reference?.ref).toBe(mockRef);
		expect(finalProp.reference?.actions?.onDelete).toBe("cascade");
	});

	test("builder should handle enum configuration", () => {
		const prop = new Property("enum");
		const builder = makeBuilder(prop);

		const enumProp = builder({ options: ["A", "B"] });
		expect(enumProp.type).toBe("enum");
		expect(enumProp.enumConfigs).toEqual({ options: ["A", "B"] });
	});
});
